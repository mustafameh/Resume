...
\vspace{0.05 cm}
        ...
        \textit{June 2025 – Present}}
            \textbf{Applied Scientist \& Applied Research Intern}
...
\vspace{0.05 cm}
        \begin{onecolentry}
            \begin{highlights}
                \item Initially hired as a 4-month intern; extended twice to 18 months based on performance to take on full-time Applied Scientist responsibilities.
                \item Building tool-augmented multi-agent systems for cross-document risk assessment and extraction over high-stakes legal documents, supporting due diligence in M\&A transactions.
                \item Designed an adversarial multi-agent evaluation pipeline that reverse-engineered expert grading criteria from audit sheets, achieving 90\%+ failure detection with 0.88 Pearson correlation to expert grades to reducing the evaluation feedback loop from days to hours and enabling rapid iteration on solution development.
                \item Fine-tuned LLMs for specialized legal use cases including long-form contract drafting, customization, and summarization.
            \end{highlights}
        \end{onecolentry}
...
\vspace{0.06 cm}
        \begin{onecolentry}
            \begin{highlights}
                \item Improved tool usage and function-calling capabilities of advanced reasoning LLMs through RLHF training pipelines, enabling models to infer, chain, and autonomously execute multi-step complex actions from high-level user intent, powering AI solutions for tech firms including Google and Meta.
                \item Curated and cleaned training datasets in collaboration with domain experts, achieving a 20\% improvement in instruction adherence.
            \end{highlights}
        \end{onecolentry}
...
\vspace{0.06 cm}
        ...
        \begin{onecolentry}
            \begin{highlights}
                ...
                \item Trained predictive models for neurological and mental health disorder diagnosis, achieving 80\% accuracy for Parkinson's and 82\% for Schizophrenia from EEG signals, and 92\% for Autism from eye-tracking data.
            \end{highlights}
        \end{onecolentry}  
...
\end{onecolentry}
    ...
    \vspace{0.05 cm}
    \textbf{Sherlock-AI: Character Emulation with LLMs} 
    
    \begin{twocolentry}{ 
            \vspace{0.10 cm} 
    {\href{https://agentic-sherlock.vercel.app/}{Website} \href{https://github.com/mustafameh/Sherlock-LLM}{Repo}}{}
        
        }
            {\textit{Hugging Face Transformers, LoRA, Flask}}
        \end{twocolentry}
...
\begin{onecolentry}
    \begin{highlights}
        \item Designed a resource-efficient, transferable character emulation pipeline: fine-tuned a quantized LLaMA model with \textit{LoRA} for Sherlock Holmes emulation, automating dialogue extraction from public domain texts and deploying on \textit{RunPod}.
        
        \item Created a web chat application for Sherlock role-play, integrating the fine-tuned model and external language models via \textit{OpenRouter} with text-to-speech, character customizability and PostgreSQL-based chat management.
        
        \item   In A/B testing, 72.4\% of users favoured fine-tuned model over baseline for authenticity, with perfect usability scores.
    \end{highlights}
      \end{onecolentry}  
\vspace{0.2 cm}
...
\begin{onecolentry}
\textbf{Programming Languages:} Python, C, Java, JavaScript, R, Haskell, SQL, MATLAB, HTML/CSS
\end{onecolentry}

\vspace{0.1cm}
\begin{onecolentry}
\textbf{Libraries \& Frameworks:}
TensorFlow, PyTorch, Hugging Face (Transformers, trl), scikit-learn, LangChain,
NLTK, spaCy, Pydantic AI, OpenCV, Flask, REST APIs,  matplotlib, seaborn, plotly, D3.js, Claude Agent SDK
\end{onecolentry}

\vspace{0.1cm}
\begin{onecolentry}
\textbf{Tools \& Platforms:}
Git (GitHub, GitLab CI), Docker, AWS (SageMaker, EC2, Lambda, Bedrock),
Apache Spark/Databricks, PostgreSQL, Runpod, DigitalOcean, Power Bi, Claude Code, Cursor
\end{onecolentry}
